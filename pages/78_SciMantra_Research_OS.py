...
STAGES = [
    ("01", "Research Question", "Define the problem, question and scope", "🎯"),
    ("02", "Literature", "Map evidence, methods and research gaps", "📚"),
    ("03", "Hypothesis", "Forge testable hypotheses and predictions", "💡"),
    ("04", "Experiment", "Design experiments, controls and power", "🧪"),
    ("05", "Data", "Capture datasets, provenance and quality", "🗃️"),
    ("06", "Analysis", "Choose, audit and stress-test analyses", "📊"),
    ("07", "Evidence", "Synthesize evidence and contradictions", "🔗"),
    ("08", "Claims", "Stress-test scope, support and novelty", "🛡️"),
    ("09", "Manuscript", "Build evidence-grounded scientific writing", "📝"),
    ("10", "Peer Review", "Attack weaknesses and manage responses", "🧐"),
    ("11", "Submission", "Audit journal requirements and readiness", "📤"),
    ("12", "Next Study", "Turn findings into the next research cycle", "🔄"),
]
...
with st.sidebar:
    ...
    st.divider()
    st.caption("Use the stage cards to maintain the current project state. Existing SciMantra tools remain available as specialist workbenches.")
...
a.metric("Project completion", f"{progress}%")
b.metric("Complete stages", complete)
c.metric("Ready next", ready)
d.metric("Blocked", blocked)
st.progress(progress / 100)

st.subheader("Research lifecycle")
cols = st.columns(4)
for i, (num,name,desc,icon) in enumerate(STAGES):
    with cols[i % 4]:
        status = st.session_state.os_status[name]
        st.markdown(f'<div class="stage"><div class="num">{num} · {status.upper()}</div><h3>{icon} {name}</h3><p>{desc}</p></div>', unsafe_allow_html=True)
        new = st.selectbox("Status", ["Not started","In progress","Blocked","Ready","Complete"], index=["Not started","In progress","Blocked","Ready","Complete"].index(status), key=f"status_{num}", label_visibility="collapsed")
        st.session_state.os_status[name] = new

st.divider()
st.subheader("Research OS control layer")
signals = [
    ("🔗 Evidence provenance", "Trace claims back to results, analyses, datasets and raw evidence."),
    ("🛡️ Scientific integrity", "Check contradictions, unsupported claims and verification gaps."),
