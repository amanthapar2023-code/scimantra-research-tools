...
if not require("automated_reports"):
    st.stop()
...
if df is not None:
    ...
    st.session_state.report_stats = numeric.describe().T if not numeric.empty else pd.DataFrame()
...
if st.button("Generate complete report", type="primary"):
    if df is None:
        ...
        lines = [title, "=" * len(title), f"Author / laboratory: {author or 'Not specified'}", f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}", "", "ABSTRACT / SUMMARY", results or "Draft this section after reviewing the statistical evidence.", "", "1. OBJECTIVE", objective or "Not specified.", "", "2. METHODS", methods or "Not specified.", "", "3. DATASET AND QUALITY", f"The dataset contained {len(df):,} observations and {len(df.columns):,} variables. There were {missing:,} missing cells. {len(numeric.columns):,} numeric variables were identified."]
        ...
        lines += ["", "4. RESULTS", results or "Not specified.", "", "5. DISCUSSION", discussion or "Not specified.", "", "6. CONCLUSION", conclusion or "Not specified.", "", "7. LIMITATIONS", limitations or "Not specified.", "", "SCIENTIFIC REVIEW NOTE", "This report is an editable draft. Verify all calculations, assumptions, experimental design, statistical choices, biological interpretations, citations and journal-specific formatting before submission."]
