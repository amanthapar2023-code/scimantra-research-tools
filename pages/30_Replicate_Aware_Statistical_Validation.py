...
missing_response = int(work[response].isna().sum())
...
# When time is selected, tests must still have one value per experimental unit.
# Collapse unit × time summaries to one unit-level mean before inferential tests.
unit_level = summary.groupby([group, replicate], dropna=False)["mean"].mean().reset_index(name="mean")
unit_summary = unit_level.groupby(group, dropna=False)["mean"].agg(
    n_units="size", mean="mean", sd=lambda x: x.std(ddof=1),
...
unit_counts = unit_level.groupby(group, dropna=False)[replicate].nunique().reset_index(name="units")
issues = []
if repeated_units:
    issues.append(f"{repeated_units} experimental units contain repeated observations; raw rows are not treated as independent replicates.")
if len(unit_counts) and unit_counts["units"].min() < 2:
    issues.append("At least one group has fewer than 2 experimental units; inferential testing is not reliable for that group.")
if missing_response:
    issues.append(f"{missing_response} rows have missing/non-numeric response values and were excluded.")
if issues:
...
st.markdown("**Analysis rule:** repeated measurements are summarized within each experimental unit first. If time is selected, the unit-level response is the mean across that unit's available time points, so time points are not counted as independent replicates.")
...
if len(groups) < 2:
    ...
    st.stop()
arrays = [unit_level.loc[unit_level[group] == g, "mean"].dropna().to_numpy(dtype=float) for g in groups]
if len(groups) == 2:
    a, b = arrays
    ...
    cohend = (np.mean(a)-np.mean(b))/pooled if np.isfinite(pooled) and pooled != 0 else np.nan
...
for g, arr in zip(groups, arrays):
    sh = stats.shapiro(arr) if 3 <= len(arr) <= 5000 else None
    diag.append({"group":g,"n_units":len(arr),"Shapiro W":sh.statistic if sh else np.nan,"normality p":sh.pvalue if sh else np.nan})
if len(arrays) >= 2:
...
report = pd.DataFrame({"Item":["Rows supplied","Rows analyzed","Experimental units","Groups","Response","Grouping variable","Replicate variable","Time variable","Analysis principle"],"Value":[len(df),len(work),int(unit_level.shape[0]),len(groups),response,group,replicate,time,"One summarized value per experimental unit; repeated observations are not independent replicates."]})
