...
if not require("power_analysis"):
    st.stop()

mode = st.selectbox("Planning module", ["Two-group sample size", "Two-group power", "Effect size from pilot data", "Randomization planner", "Experimental design checklist"])
...
if mode == "Two-group sample size":
    ...
    if st.button("Calculate power", type="primary"):
        ...
        if power < 0.80: st.warning("Planned power is below 80%. Consider increasing replication or reassessing the expected effect size.")
        else: st.success("Planned power is at or above 80% under the stated assumptions.")
    ...
    if st.button("Calculate effect size", type="primary"):
        try:
            ...
            if len(va) < 2 or len(vb) < 2: raise ValueError("Each group needs at least two observations.")
        ...
        except Exception as exc: st.error(f"Could not calculate effect size: {exc}")
    ...
    if st.button("Generate allocation", type="primary"):
        try:
            ...
            labels, counts = [], []
            for part in parts:
                name, count = part.rsplit(":", 1); labels.append(name.strip()); counts.append(int(count.strip()))
            allocation = [name for name, count in zip(labels, counts) for _ in range(count)]
            rng = np.random.default_rng(int(seed)); rng.shuffle(allocation)
        ...
        except Exception as exc: st.error(f"Could not create allocation: {exc}")
else:
    st.subheader("Experimental design checklist")
    items = ["Define the primary research question and primary outcome before data collection.", "Identify the true experimental unit; technical replicates are not automatically independent biological replicates.", "Predefine treatment groups, controls and inclusion/exclusion criteria.", "Choose biological replication based on expected variability and scientifically meaningful effect size.", "Randomize assignment where feasible and document the randomization method.", "Consider blocking/stratification when batch, day, operator or other nuisance factors may influence outcomes.", "Predefine the statistical model and primary comparison where possible.", "Plan how missing observations, outliers and failed experiments will be handled.", "Avoid increasing sample size solely because an interim p-value is inconvenient unless a valid sequential design was planned.", "Record protocol deviations and preserve an auditable analysis dataset."]
    for i, item in enumerate(items): st.checkbox(item, key=f"design_{i}")
