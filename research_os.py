...
The explicit navigation below is intentional: the repository contains legacy page
filenames with duplicate numeric prefixes (for example two different 20_*, 30_*,
31_*, etc.). Streamlit's automatic pages-directory navigation normalizes those
names to the same URL and can fail while building navigation. Explicit Page
objects use the full filename as a stable, unique URL path.
"""
...
def _url_path(path: Path) -> str:
    # Full stem keeps legacy duplicate numeric prefixes distinct.
    return re.sub(r"[^a-z0-9_-]+", "-", path.stem.lower()).strip("-")


def _build_pages():
    files = sorted(Path("pages").glob("*.py"), key=lambda p: p.name.lower())
    research_os = [p for p in files if re.match(r"^(100|101|102|103|104|105|106|107|108|109|110|111|112|113|114|115|116|118|119|120)_", p.name)]
    ...
    return {
        "SciMantra Research OS": [
            ...
            *[st.Page(p, title=_page_title(p), icon="🧠", url_path=_url_path(p)) for p in research_os],
        ],
        "Research Tools": [
            *[st.Page(p, title=_page_title(p), icon="🧪", url_path=_url_path(p)) for p in core],
        ],
    }
...
def _home():
    ...
    page_map = {p.stem: p for p in Path("pages").glob("*.py")}
